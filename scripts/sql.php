...
if (!$link)
	errorForm('Unable to connect to the database server.', 'HTTP/1.1 500 Internal Server Error');

// Ensuring correct encoding 
if (!mysqli_set_charset($link, 'utf8'))
	errorForm('Unable to set database connection encoding.', 'HTTP/1.1 500 Internal Server Error');

// Selecting the correct database
if(!mysqli_select_db($link, $dbname))
	errorForm('Unable to locate the database.', 'HTTP/1.1 500 Internal Server Error');
// =======================================================
...
$alltables = array("temperature", "lux", "humidity", "smoke", "water_leakage", "occupancy", "co2");
$JSONtable = array();
...
if($POST_LOC)
{
	...
	{
		$JSONtable[] = array("sensorID" => $row['deviceID'], "floor" => (int)$row['floor'], 
...
if($POST_NOTIFICATION)
{
	...
	if ($result->num_rows != 0)	// If there is a row in Failures, then there is a failure. Unless the SQL database fails. Or maybe they both failed?
	{
		...
		{
			$JSONtable["failure"][] = array("sensorID" => $row['sensorID'], "floor" => $row['floor'], "located" => $row['located'], "type" => $row['type'], "timestamp" => strtotime($row['last_seen']) );
		}
	}
	...
	if ($result->num_rows != 0)	// If there is a row in Failures, then there is a failure. Unless the SQL database fails. Or maybe they both failed?
	{
		while($row = mysqli_fetch_assoc($result))	// Print the data in the row with HTML
		{
			$JSONtable["isolation_failure"][] = array("sensorID" => $row['sensorID'], "floor" => $row['floor'], "located" => $row['located'], "type" => $row['type'] );
		}
	}
	...
	if ($result->num_rows != 0)	// If there is a row in Failures, then there is a failure. Unless the SQL database fails. Or maybe they both failed?
	{
		while($row = mysqli_fetch_assoc($result))	// Print the data in the row with HTML
		{
			$JSONtable["isolation_warning"][] = array("sensorID" => $row['sensorID'], "floor" => $row['floor'], "located" => $row['located'], "type" => $row['type'] );
		}
	}
	ob_end_clean();
	echo json_encode($JSONtable);
...
/*********************************GENERIC QUERY - NO SENSOR ID *******************************/
	...
	//Form sub date query
	$date_query = "AND timestamp BETWEEN '$datefrom' AND '$dateto' ";
	
	/******************************** END DATES ********************************/
	
	//Start query
	$query = "SELECT* FROM repp JOIN nodes ON repp.sensorID = nodes.deviceID WHERE value!= 9999 " . $date_query;
...
function PrintSingleTable($queryresult, $table)
{
	...
	{
		$JSONtable[$table][] = array("sensorID" => $row['sensorID'], "floor" => (int)$row['floor'], 
			"location" => $row['located'], "timestamp" => strtotime($row['timestamp']), "value" => (double)$row['value'], "status" => $row['status']);
	}
}

function errorForm($output, $header='HTTP/1.1 400 Bad Request')
{
	header($header);
	echo $output;
	exit();
